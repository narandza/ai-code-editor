...
export const createScraperUrlTool = () => {
  return createTool({
    name: "scrapeUrls",
    description:
      "Scrape content from URLs to get documentation or reference material. Use this when user provides URLs or references external documentation. Returns markdown content form the scraped pages.",
    parameters: z.object({
        ...
        return await toolStep?.run("scrape-urls", async () => {
          ...
          for (const url of urls) {
            try {
              ...
              results.push({
                url,
                content: `Failed to scrape URL: ${ur}`,
              });
            }
          }
