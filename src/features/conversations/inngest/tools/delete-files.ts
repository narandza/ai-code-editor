...
export const createDeleteFilesTool = ({
  ...
  return createTool({
    name: "deleteFiles",
    description:
      "Delete file or folders from the project. If deleting a folder, all contents will be deleted recursively.",
    parameters: z.object({
      fileIds: z
        .array(z.string())
        .describe("Array of file or folderIDs to delete"),
    }),
    handler: async (params, { step: toolStep }) => {
      ...
      // Validate all files exist before running step
      const filesToDelete: {
        id: string;
        name: string;
        type: string;
