...
export const exportToGithub = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Build a map of file IDs of their full paths
    ...
    // Create the tree
    const { data: tree } = await step.run("create-tee", async () => {
    ...
    // Set status completed with repo URL
