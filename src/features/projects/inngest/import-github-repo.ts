...
export const importGithubRepo = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Cleanup any existing file in the project
    ...
    // Sort folders by dept so parents are created before children
    // Input: [{path: "src/components"}, {path: "src"}, {path: "src/component/ui"}]
    // Output: [{path: "src"},{path: "src/components"}, {path: "src/component/ui"}]
